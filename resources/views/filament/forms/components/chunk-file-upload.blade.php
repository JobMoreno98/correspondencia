...
>
    <div 
        x-data="{
            ...
            fileName: '',
            init() {
                // Solo inicializamos Resumable si el componente NO está deshabilitado (View)
                if (! {{ $isFieldDisabled() ? 'true' : 'false' }} && typeof Resumable !== 'undefined') {
                    ...
                    if (this.$refs.fileInput) {
                        resumable.assignBrowse(this.$refs.fileInput);
                    }
                    ...
                    resumable.on('fileError', (file, message) => {
                        this.uploading = false;
                        alert('Error al subir el fragmento: ' + message);
                    });
                }
            }
        }"
            ...
            <div x-show="!state && !{{ $isFieldDisabled() ? 'true' : 'false' }}">
                <div class="flex items-center justify-center w-full">
                    <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-xl cursor-pointer border-gray-300 bg-gray-50/50 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50 dark:hover:bg-gray-900 transition duration-75">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4">
                            <svg class="w-8 h-8 mb-3 text-gray-400 dark:text-gray-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 16.5V9.75m0 0 3 3m-3-3-3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.233-2.33 3 3 0 0 1 3.758 3.848A3.752 3.752 0 0 1 18 19.5H6.75Z" />
                            </svg>
                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                <span class="font-semibold text-custom-600 dark:text-custom-400">Haz clic para seleccionar</span> o arrastra tu archivo grande
                            </p>
                        </div>
                ...
                <div x-show="uploading" class="w-full mt-3" x-cloak>
                    <div class="w-full bg-gray-200 rounded-full h-2 dark:bg-gray-700 overflow-hidden">
                        <div class="bg-custom-600 h-2 rounded-full transition-all duration-150" :style="`width: ${progress}%`"></div>
                    </div>
                    <span class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-1.5 block" x-text="`Subiendo ${fileName}: ${progress}%`"></span>
                </div>
            </div>

            <div x-show="!state && {{ $isFieldDisabled() ? 'true' : 'false' }}" class="text-sm italic text-gray-500 dark:text-gray-400 p-2">
                Sin archivo adjunto.
            </div>
                ...
                <div class="flex items-center justify-between p-3 border rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
                    <div class="flex items-center gap-x-3 truncate">
                        ...
                        <div class="flex flex-col truncate">
                            <span class="text-sm font-medium text-gray-950 dark:text-white truncate" x-text="state.split('/').pop().replace(/^\d+_(.+)$/, '$1')"></span>
                    ...
                    <div class="flex items-center gap-x-2 pl-2">
                        ...
                        @if(! $isFieldDisabled())
                            @can('delete', $component)
                                <button 
                                    type="button"
                                    @click="if(confirm('¿Estás seguro de quitar este archivo?')) { state = null; }"
